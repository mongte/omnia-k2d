...
export const EventDetailView = ({ event, onBack }: EventDetailViewProps) => {
  const [activeSection, setActiveSection] = useState<'header' | 'description' | 'tasks' | 'time' | null>(null);
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.titleSection}>
          ...
          <View style={styles.titleContent}>
            {activeSection === 'header' ? (
                <View>
                    <TextInput 
                        style={[styles.title, { borderBottomWidth: 1, borderColor: '#ccc', paddingBottom: 0 }]}
                        value={title}
                    ...
                    />
                    <TextInput 
                        style={[styles.subtitle, { borderBottomWidth: 1, borderColor: '#ccc', marginBottom: 8 }]}
                        value={subtitle}
                ...
                <TouchableOpacity onPress={() => setActiveSection('header')} activeOpacity={0.7}>
                    <Text style={styles.title}>{title}</Text>
                    <View style={styles.subtitleContainer}>
                        {!!subtitle && <Text style={styles.subtitle}>{subtitle}</Text>}
                        {!subtitle && <Text style={[styles.subtitle, { color: '#cbd5e1' }]}>Add subtitle</Text>}
        ...
        {activeSection === 'description' ? (
             <TextInput 
                style={[styles.description, { minHeight: 100, textAlignVertical: 'top', borderWidth: 1, borderColor: '#f1f5f9', borderRadius: 12, padding: 12 }]}
                value={description}
            ...
            <TouchableOpacity onPress={() => setActiveSection('description')} activeOpacity={0.7}>
                <Text style={styles.description}>
                {isLoading ? 'Loading details...' : (description || 'No description provided')}
                </Text>
            </TouchableOpacity>
        )}
          ...
          <View style={styles.subtasksHeader}>
            ...
            <View style={styles.counterBadge}>
              <Text style={styles.counterText}>
                  {displayEvent.subtasks.filter((t: any) => t.is_completed).length}/{displayEvent.subtasks.length}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.checklist}>
            {activeSection === 'tasks' ? (
                <>
                        ...
                        <View key={task.id || index} style={styles.checkItem}>
                            <TouchableOpacity onPress={() => {
                                ...
                                <View style={[styles.checkbox, task.is_completed && styles.checkedBox]}>
                                    {task.is_completed && <MaterialIcons name="check" size={14} color="#fff" />}
                                </View>
                            </TouchableOpacity>
                            <TextInput
                                style={[styles.checkText, { flex: 1, borderBottomWidth: 1, borderColor: '#eee' }]}
                                value={task.title}
                                onChangeText={(text) => {
                                    const newTasks = [...subtasks];
                    ...
                    <TouchableOpacity 
                        style={[styles.checkItem, { justifyContent: 'center', borderStyle: 'dashed', borderWidth: 1, borderColor: '#ccc' }]}
                        onPress={() => {
                            setSubtasks([...subtasks, { id: `temp-${Date.now()}`, title: '', is_completed: false, isNew: true }]);
                        }}
                    >
                ...
                <TouchableOpacity onPress={() => setActiveSection('tasks')} activeOpacity={0.7} style={{ width: '100%' }}>
                    {isLoading && <ActivityIndicator size="small" color="#FF9800" />}
                    {!isLoading && subtasks.map((task: any) => (
                        <View key={task.id} style={styles.checkItem}>
                            <View style={[styles.checkbox, task.is_completed && styles.checkedBox]}>
                                {task.is_completed && <MaterialIcons name="check" size={14} color="#fff" />}
                            </View>
                            <Text style={[styles.checkText, task.is_completed && styles.checkedText]}>{task.title}</Text>
                        </View>
                    ))}
                    {!isLoading && subtasks.length === 0 && (
                        <Text style={{color: '#94a3b8', fontStyle: 'italic', padding: 8}}>No subtasks</Text>
                    )}
                </TouchableOpacity>
            )}
        ...
        <View style={{ marginBottom: 24 }}>
            <View style={[styles.timeBanner, { backgroundColor: event.color }]}>
                <View style={styles.timeBannerHeader}>
                  {(() => {
                      ...
                      if (isSameDay) {
                          return (
                              <>
                                <Text style={styles.timeDate}>{format(start, 'EEEE, MMM d').toUpperCase()}</Text>
                                <Text style={styles.timeDate}>{format(start, 'yyyy')}</Text>
                              </>
                          );
                      } else {
                          return (
                ...
                <View style={styles.timeDisplay}>
                  {activeSection === 'time' ? (
                      ...
                      {/* Start Time Row */}
                      <View>
                        <Text style={{color: 'rgba(255,255,255,0.7)', fontSize: 12, marginBottom: 4}}>Start</Text>
                        <View style={{ flexDirection: 'row', gap: 12 }}>
                            <TouchableOpacity 
                                style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 8, borderBottomWidth: 1, borderColor: 'rgba(255,255,255,0.3)' }}
                                onPress={() => {
                            ...
                            <TouchableOpacity 
                                style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 8, borderBottomWidth: 1, borderColor: 'rgba(255,255,255,0.3)' }}
                                onPress={() => {
                      ...
                      {/* End Time Row */}
                      <View>
                        <Text style={{color: 'rgba(255,255,255,0.7)', fontSize: 12, marginBottom: 4}}>End</Text>
                        <View style={{ flexDirection: 'row', gap: 12 }}>
                            <TouchableOpacity 
                                style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 8, borderBottomWidth: 1, borderColor: 'rgba(255,255,255,0.3)' }}
                                onPress={() => {
                    ...
                    <TouchableOpacity onPress={() => setActiveSection('time')} activeOpacity={0.7}>
                        <View style={styles.startTime}>
                            <MaterialIcons name="schedule" size={24} color="#fff" />
                            <Text style={styles.bigTime}>{format(new Date(startTime || event.startTime), 'hh:mm a')}</Text>
                        </View>
                        <View style={styles.timeDivider} />
                        <Text style={styles.endTime}>{format(new Date(endTime || event.endTime), 'hh:mm a')}</Text>
                    </TouchableOpacity>
                  )}
                </View>
            </View>
            <LinearGradient
                ...
                style={[StyleSheet.absoluteFill, { top: '50%', bottom: -40, opacity: 0.6, zIndex: -1 }]}
                start={{ x: 0, y: 0 }}
                end={{ x: 0, y: 1 }}
            />
        </View>
              ...
              {(() => {
                  const hasChanges = 
                      ...
                      // Simple subtask check (count or content)
                       JSON.stringify(subtasks.map(t => ({t: t.title, c: t.is_completed}))) !== 
                       JSON.stringify((fullEvent?.subtasks || []).map((t: any) => ({t: t.title, c: t.is_completed})));
      ...
      <MiniCalendarModal
        ...
        initialDate={new Date(datePickerTarget === 'start' ? startTime : endTime)}
        onDateSelect={(selectedDate) => {
            const targetTime = datePickerTarget === 'start' ? new Date(startTime) : new Date(endTime);
      ...
      <TimePickerModal
        ...
        initialTime={new Date(timePickerTarget === 'start' ? startTime : endTime)}
        onTimeSelect={(hours, minutes) => {
            const targetTime = timePickerTarget === 'start' ? new Date(startTime) : new Date(endTime);
