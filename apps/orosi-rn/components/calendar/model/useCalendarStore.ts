...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  setFocusedDay: (date, source = 'user') =>
    set({ focusedDay: date, lastUpdateSource: source }),
  ...
  toggleDaySelection: (date) => {
    ...
    if (
      selectedRange &&
      isSameDay(selectedRange.start, date) &&
      isSameDay(selectedRange.end, date)
    ) {
  ...
  startSelection: (date) =>
    set({ dragStarted: true, selectedRange: { start: date, end: date } }),
  ...
  updateSelection: (date) => {
    ...
    const start = isBefore(date, selectedRange.start)
      ? date
      : selectedRange.start;
  ...
  setDetailModal: (isOpen, date = null, range = null) =>
    set({ detailModal: { isOpen, date, range } }),
}));
...
export const selectIsDaySelected = (state: CalendarState, date: Date) => {
  ...
  return (
    (isAfter(date, state.selectedRange.start) ||
      isSameDay(date, state.selectedRange.start)) &&
    (isBefore(date, state.selectedRange.end) ||
      isSameDay(date, state.selectedRange.end))
  );
};
